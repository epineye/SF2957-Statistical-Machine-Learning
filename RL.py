...
def learn_MC(env, n_sims, gamma = 1, epsilon = 0.05,
    ...
    for episode in range(1,n_sims + 1):
     ...
     ####################################################################
        ...
        for idx, sta in enumerate(episode_state_action_count) :
            for act in episode_state_action_count[sta] :
                if episode_state_action_count[sta][act] == 1 :
                    Q[sta][act]=Q[sta][act] + (episode_reward - Q[sta][act])/(state_action_count[sta][act])
